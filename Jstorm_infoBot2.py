    ...
    if msgSTR in ("","哈囉","嗨","嗨嗨","你好","您好","在嗎","早安","午安","晚安","こんにちは","こんばんは","やっほー","やっはろ","やほー","Hi","hi","hello","Hello","安安"):
        ...
        #member
            ...
            #age
                ...
                if mscDICT[client.user.id]["member"] in tokio:
                    for n in range(len(tokio)):
                        if mscDICT[client.user.id]["member"]==ProfileDICT["TOKIO"][n]["JName"]:
                            birthday = ProfileDICT["TOKIO"][n]["Birth"].split(".")
                elif mscDICT[client.user.id]["member"] in arashi:
                    for n in range(len(tokio)):
                        if mscDICT[client.user.id]["member"]==ProfileDICT["嵐"][n]["JName"]:
                            birthday = ProfileDICT["嵐"][n]["Birth"].split(".")
                elif mscDICT[client.user.id]["member"] in kattun:
                    for n in range(len(tokio)):
                        if mscDICT[client.user.id]["member"]==ProfileDICT["KAT-TUN"][n]["JName"]:
                            birthday = ProfileDICT["KAT-TUN"][n]["Birth"].split(".")
                elif mscDICT[client.user.id]["member"] in jump:
                    for n in range(len(tokio)):
                        if mscDICT[client.user.id]["member"]==ProfileDICT["Hey! Say! JUMP"][n]["JName"]:
                            birthday = ProfileDICT["Hey! Say! JUMP"][n]["Birth"].split(".")
