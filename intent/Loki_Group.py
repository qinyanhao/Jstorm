...
import json
with open('D:\HAO\Hao的研所\實習\Jstorm\ProfilePic.json', 'r') as f:
    ProfilePic=json.load(f)

with open('D:\HAO\Hao的研所\實習\Jstorm\GroupDICT', 'r') as f:
...
def countDay(group,anniversary):
    '''
    輸入:團名,出道日/結成日
    輸出:距離下一次節日的天數
    '''
    ...
    current_anniversary= datetime.date(year=today.year, month=date[1],day=date[2])
...
def getResult(inputSTR, utterance, args, resultDICT):
    ...
    if utterance == "離[出道日]還有[幾天]":
        if args[0]=='出道日':
            ...
            resultDICT['request']='formationdat.days'
